...
class WPLMS_Front_End {
	...
	function create_course_button(){
		if(function_exists('vibe_get_option')){
			...
			if(isset($pageid) && $pageid && current_user_can('edit_posts')){
				...
				echo '<a href="'.get_permalink($pageid).'" class="button create-group-button full">'. __( 'Create a Course', 'vibe' ).'</a>';
			}
		}
	}
    ...
    function wplms_edit_course_menu_link($nav_menu){
        ...
        global $post;
            if(isset($pageid) && $pageid && ( $post->post_author == $user_id || current_user_can('manage_options'))){
                ...
                echo '<li id="edit"><a href="'.get_permalink($pageid).'?action='.$post->ID.'">'.__( 'Edit Course', 'vibe' ).'</a></li>';
        }
    }
    ...
    function save_course_settings(){
        ...
        $course_setting['vibe_certificate']=$_POST['vibe_certificate'];
    ...
    function delete_curriculum(){
        ...
        if(!isset($id) || !is_numeric($id) && $id == ''){
            _e('Can not have delete','wplms-front-end');
             die();
        }
    ...
    function wplms_front_end_unit_controls(){
        ...
        <ul class="unit_settings">
            <li><label><?php _e('Unit Type','wplms-front-end'); ?></label>
                ...
                <select id="vibe_type" class="chosen">
                    <?php
                    ...
                    foreach($unit_types as $unit){
                        echo '<option value="text-document" '. selected($unit_settings['vibe_type'],$unit['value'],false).'>'.$unit['label'].'</option>';
                    }
                    ?>
                </select>    
                </span></h3></li>
        ...
        if(isset($course_id) && is_numeric($course_id)){
            ...
            foreach($questions as $key=>$question){
                echo '<li><strong>'.__('Question ','wplms-front-end').($key+1).' : '.get_the_title($question).'</strong>
                        ...
                        <span>'.__('MARKS : ').$marks[$key].'<input type="hidden" class="question_marks" value="'.$marks[$key].'" /></span>
            ...
            <ul id="hidden">
                <li class="new_question">
                    ...
                    <span>
                        <?php _e('Marks : '); ?><input type="number" class="small_box question_marks" value="0" />
                    </span>
    ...
    function wplms_front_end_quiz_meta_controls(){
       ...
       <div class="wplms_front_end_wrapper">
        ...
        <li class="dynamic <?php echo (($quiz_settings['vibe_quiz_dynamic']=='S')?'':'hide');?>"><label><?php _e('SELECT QUESTION TAGS','wplms-front-end'); ?></label>
        ...
        <li class="dynamic <?php echo (($quiz_settings['vibe_quiz_dynamic']=='S')?'':'hide');?>"><label><?php _e('NUMER OF QUESTIONS','wplms-front-end'); ?></label>
              <input type="number" class="small_box" id="vibe_quiz_number_questions" value="<?php echo $quiz_settings['vibe_quiz_number_questions']; ?>" />
        </li>
        <li class="dynamic <?php echo (($quiz_settings['vibe_quiz_dynamic']=='S')?'':'hide');?>"><label><?php _e('MARKS PER QUESTION','wplms-front-end'); ?></label>
    ...
    function create_question(){
        ...
        echo '<strong>'.__('Question ','wplms-front-end').' : '.$question_title.'</strong>
                        ...
                        <span>'.__('MARKS : ').'<input type="number" class="question_marks small_box" value="0" /></span>
    ...
    function wplms_front_end_question_controls(){
        ...
        $question_types = apply_filters('wplms_question_types',array(
              array( 'label' =>'True or False','value'=>'truefalse'),  
              array( 'label' =>'Multiple Choice','value'=>'single'),
              array( 'label' =>'Multiple Correct','value'=>'multiple'),
              array( 'label' =>'Sort Answers','value'=>'sort'),
              array( 'label' =>'Match Answers','value'=>'match'),
              array( 'label' =>'Fill in the Blank','value'=>'fillblank'),
              array( 'label' =>'Dropdown Select','value'=>'select'),
